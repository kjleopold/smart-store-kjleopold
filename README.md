...
### 2. Data Source
* The main dataset was obtained from Kaggle: [Spotify User Behavior Survey](https://www.kaggle.com/datasets/coulsonlll/spotify-user-behavior-survey-data). The dataset is based on a user survey, so the data may not fully represent the entire user base.  
...
* Spotify can drive growth by focusing on Free plan users, especially those aged 20–35, who make up the largest group and show the most upgrade interest.
* Targeted campaigns promoting premium benefits, especially for the Individual Plan—could help convert these users.
* The Individual Plan generates the most revenue and has the greatest growth potential, making it a key focus for marketing.
* The Family Plan is a less popular upgrade choice and may need to be reevaluated or marketed differently.
* Personalized messaging and in-app nudges based on users’ current plans and preferences could help boost subscription conversions and close the revenue gap.
### 7. Challenges
* The dataset didn’t include any date fields, so I had to get creative with slicing and drill-downs to showcase analytical skills.
...
* Privacy Considerations: Privacy is essential when working with user data. Even if personal identifiers like names are not included, combining certain data points can still risk identifying individuals, especially in smaller groups. In my analysis, I ensured that the data was not connected in ways that could identify individual users.
* Bias Awareness: Bias can significantly affect the fairness of insights. Overrepresentation of specific age groups or plans may skew conclusions and lead to unbalanced decisions. A larger, more diverse sample may be needed before making any critical business moves.
